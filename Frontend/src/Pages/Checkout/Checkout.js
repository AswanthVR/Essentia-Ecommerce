...
function Checkout() {
  emailjs.init('bs5GyhirClfjx_O7B');  
    let totalAmount = 0;
  ...
  useEffect(() => {
    ...
    , getAuthorizationHeaders())
        .then(response => {
          ...
          console.log(response.data.oid);
          postPaymentDeails(response.data.oid , response.data.orderTotal , paymentId);
          nav(`/order_summary/${response.data.oid}`)
    ...
    const postPaymentDeails = (oid, amount , paymentId) => { 
      const paymentDate = new Date(); 
      console.log(paymentDate.toISOString());
      console.log(oid , amount , paymentType);
...
useEffect(() => {
  ...
  setRazorpay(rzp);
}, [totalAmount, firstName, lastName, email, phoneNumber, address]);
...
const handleRazorpayResponse = (response) => { 
  ...
  return (
    <>
    <Navbar12/>
    {(msg)&&<div className='animate-[fadeInDown_1s]  overflow-hidden '>
